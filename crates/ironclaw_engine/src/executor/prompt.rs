...
fn build_codeact_system_prompt_inner(
    ...
    // Layout for prompt caching: stable-per-deployment prefix
    ...
    // Tool list (dynamic: changes with lease set). Last so lease changes
    // invalidate only the tail, not the whole cacheable prefix.
    if !actions.is_empty() {
