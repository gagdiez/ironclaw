...
#[tokio::test]
#[ignore = "migrate: rewrite mock to use LlmResponse::Code with `await <tool>()` — see testing::code_final; gate mechanism itself is covered by engine-crate tests"]
...
// gate_resume_with_execution_obligation: removed. The execution-obligation
// nudge system (enable_tool_intent_nudge / require_action_attempt /
// max_action_requirement_nudges) was deleted as part of the code-only
// coherence work — a heuristic that couldn't distinguish intent from
// content, replaced by the model seeing real errors and self-correcting.
